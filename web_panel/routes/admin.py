...
def _build_transfer_users_preview(source_server: Server, target_server: Server) -> dict[str, int]:
    source_users = (
        VpnUser.query
        .options(load_only(VpnUser.id, VpnUser.username, VpnUser.is_active, VpnUser.server_id))
...
def _background_transfer_server_users(
    ...
    with app.app_context():
        ...
        try:
            for batch_index, start in enumerate(range(0, total, _SERVER_DELETE_SYNC_BATCH_SIZE), start=1):
                ...
                for uid in batch_ids:
                    with db.session.no_autoflush:
                        ...
                        elif user.reseller and int(user.reseller.server_id or 0) != int(target_server.id):
                            owner = ensure_target_admin_owner()
                            user.reseller_id = owner.id
                            ownership_adjusted += 1
                ...
                _update_delete_sync_status(
                    ...
                    message=(
                        ...
                        f'inactivos={moved_inactive}, conflictos={skipped_conflicts}, fallidos={failed}.'
                    ),
                    processed_users=processed_users,
                    created=moved_active,
                    already_existed=moved_inactive,
                    failed=failed,
                    total_batches=total_batches,
                    current_batch=batch_index,
                )
            ...
            message = (
                f"Transferencia completada: activos={moved_active}, inactivos={moved_inactive}, "
                f"propietarios_ajustados={ownership_adjusted}, conflictos={skipped_conflicts}, "
...
def _start_transfer_users_background_sync(
    ...
    app = current_app._get_current_object()
    sync_id = _create_delete_sync_status(
        source_server_name=f"{source_server.name} (transfer)",
...
def servers():
    ...
    all_servers = Server.query.order_by(Server.id.asc()).all()
...
def transfer_server_users(server_id: int):
    ...
    try:
        ...
        if not source_user_ids:
            flash(f"No hay usuarios para transferir desde '{source_server.name}'.", 'info')
            return redirect(url_for('admin.servers'))

        sync_id = _start_transfer_users_background_sync(source_server, target_server, source_user_ids)
        flash(
            (
                f"Transferencia encolada '{source_server.name}' -> '{target_server.name}'. "
...
def delete_reseller(reseller_id: int):
    ...
    if not db.session.get(Server, default_server_id):
        fallback_server = Server.query.filter_by(is_active=True).order_by(Server.id.asc()).first()
        if fallback_server:
